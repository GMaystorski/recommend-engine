...
public class RecommendService {
	...
	public Map<Production, Double> processInput(List<String> actorNames, List<String> movieTitles,
		...
		processCategories(categories, recommendations);
		return recommendations;
	}

	private void processCategories(List<String> categories, Map<Production, Double> recommendations) {
		Map<Production, Integer> multipliers = recommendations.keySet().stream()
				.collect(Collectors.toMap(production -> production,
						production -> CollectionUtils.intersection(production.getCategories(), categories).size()));
		multipliers.entrySet().stream().forEach(
				entry -> recommendations.compute(entry.getKey(), (k, v) -> v * (1 + (entry.getValue() * 0.2))));
	}
	...
	private void processMovieTitles(List<String> movieTitles, Map<Production, Double> recommendations) {
		...
		try {
			...
			executeQuery(decadeCategoryMoviesQuery, record -> processGenericMovieResult(record, recommendations),
					session);
			sessionPool.releaseTarget(session);
		} catch (ConnectionAcquisitionException e) {
			// TODO Auto-generated catch block
	...
	private String buildDecadeCategoryMoviesQuery(String titles) {
		...
		return new CypherQueryBuilder().match().withNode("v5", "Category", Collections.emptyMap())
				.withEdge(EdgeDirection.DESTINATION_TO_SOURCE).withNode("v1", "Production", Collections.emptyMap())
				.withEdge(EdgeDirection.SOURCE_TO_DESTINATION).withNode("v2", "Year", Collections.emptyMap()).newLine()
				.match().withNode("v3", "Year", Collections.emptyMap()).withEdge(EdgeDirection.DESTINATION_TO_SOURCE)
				.withNode("v", "Production", Collections.emptyMap()).withEdge(EdgeDirection.SOURCE_TO_DESTINATION)
				.withNode("v4", "Category", Collections.emptyMap()).newLine()
				.where("v1.title IN " + titles, "v.title <> v1.title").newLine().with(firstVars).newLine()
				.where("v4.name in c1").newLine().with(secondVars).newLine()
				.where("decadeStart <= toInteger(v3.year) <= decadeStart + 10").newLine().returnValues("v", "c")
				.build();
	...
	private String buildActorMoviesQuery(String actors, String titles) {
		...
		return new CypherQueryBuilder().match().withNode("v2", "Category", Collections.emptyMap())
				.withEdge(EdgeDirection.DESTINATION_TO_SOURCE).withNode("v", "Production", Collections.emptyMap())
				.withEdge("", "actedIn", EdgeDirection.DESTINATION_TO_SOURCE)
				.withNode("v1", "Person", Collections.emptyMap())
				.where("v1.name IN " + actors, "NOT v.title IN " + titles).with(vars).returnValues("v", "c").build();
	...
	private String buildDirectorMoviesQuery(String titles) {
		...
		return new CypherQueryBuilder().match().withNode("v3", "Category", Collections.emptyMap())
				.withEdge(EdgeDirection.DESTINATION_TO_SOURCE).withNode("v", "Production", Collections.emptyMap())
				.withEdge("", "directed", EdgeDirection.DESTINATION_TO_SOURCE)
				.withNode("v1", "Person", Collections.emptyMap())
				.withEdge("", "directed", EdgeDirection.SOURCE_TO_DESTINATION)
				.withNode("v2", "Production", Collections.emptyMap()).newLine().with(vars)
				.where("v2.title IN " + titles, "v.id <> v2.id").returnValues("v", "c").build();
	}
	...
	private String buildSimilarityQuery(String titles) {
		...
		return new CypherQueryBuilder().match().withNode("v1", "Production", Collections.emptyMap()).newLine().match()
				.withNode("v", "Production", Collections.emptyMap()).withEdge(EdgeDirection.SOURCE_TO_DESTINATION)
				.withNode("v2", "Category", Collections.emptyMap()).newLine()
				.where("v1.title IN " + titles, "v.title <> v1.title").newLine().with(vars).newLine()
				.where("similarity >= 0.8", "similarity < 1.0").newLine().returnValues("v", "c", "similarity")
				.orderBy("similarity", OrderDirection.DESC).build();
	}
	...
	private Production mapProductionFromRecord(Record record) {
		...
		return new Production(productionProps.get("id").toString(), productionProps.get("description").toString(),
				productionProps.get("title").toString(), categories);
	}
